    ...
    <main class="w-full max-w-5xl mx-auto px-4 sm:px-6 lg:px-12 pt-24 pb-12">
      ...
      <template v-else>
        ...
        <div class="grid gap-4 grid-cols-1 sm:grid-cols-3 mb-8">
          ...
          >
            ...
            <p
              ...
              :class="
                (profile?.stats.absent_sessions ?? 0) > 0
                  ? 'text-amber-500'
                  : 'text-foreground'
              "
            >
              {{ profile?.stats.absent_sessions ?? 0 }}
            </p>
          </div>
        </div>
        ...
        >
          ...
          <p
            v-else-if="
              profile?.next_session && !profile?.profile.google_meet_link
            "
        ...
        <div class="border border-border/50 rounded-xl p-6 bg-card">
          <div class="flex items-center gap-3 mb-5">
            ...
            <span
              class="ml-auto font-body text-xs text-muted-foreground italic"
            >
...
const canJoinSession = computed(() => {
  ...
  if (!profile.value.profile.google_meet_link) return false;
...
async function loadData() {
  ...
  try {
    const dashData = await getClientDashboard();

    profile.value = dashData;
    notes.value = dashData.notes;
  } finally {
